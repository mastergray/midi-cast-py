...
class MIDICastXMLNote:

    '''Transforms NOTE element into a MIDNoteMessageOn message'''
    ...
    def __init__(self, elem):
        if elem.tag == "note":
            ...
            self.note = elem.attrib.get("tone")
            self.gate = elem.attrib.get("gate", None)
            self.velocity = elem.attrib.get("velocity", None) 
        else:
            raise ValueError(f"Expected NOTE tag but recieved '{elem.tag}' instead")
    ...
    def __call__(self, messages) -> None:
        ...
        message = MIDINoteMessageOn(channel=self.channel, note=self.note, gate=self.gate, velocity=self.velocity)
